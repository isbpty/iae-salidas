...
function render() {
  ...
  renderTabs();
  if (UI.split) {
    const left = UI.view === 'parents' ? viewParents() : viewWhatsapp();
    main.className = 'split';
    main.innerHTML = '<div class="pane">' + left + '</div><div class="pane">' + viewSchool() + '</div>';
  } else {
  ...
  // re-render cuando termine "escribiendo..."
  const now = Date.now();
  const pend = Object.values(S.chats).flat().filter((m) => m.pendingUntil && m.pendingUntil > now).map((m) => m.pendingUntil);
...
function renderTabs() {
  const tabs = [['parents', '📱 App Padres'], ['whatsapp', '💬 WhatsApp'], ['school', '🏫 Escuela'], ['log', '📜 Bitácora']];
  document.getElementById('tabs').innerHTML = tabs.map(([k, l]) => '<button class="tab' + (UI.view === k ? ' active' : '') + '" data-action="setView" data-view="' + k + '">' + l + '</button>').join('');
}
...
function viewParents() {
  const accounts = S.persons.filter((p) => p.account);
  if (!person(UI.parentId) || !person(UI.parentId).account) UI.parentId = accounts[0].id;
  const p = person(UI.parentId);
  ...
  return '<div class="phone-wrap"><div class="sim-bar">👤 Simular padre/madre: <select data-change="setParent">' +
    accounts.map((a) => opt(a.id, a.name + ' (' + a.relation + ')', a.id === p.id)).join('') + '</select></div>' +
    '<div class="phone app">' +
    '<div class="app-header"><div><div class="muted small">' + esc(S.school.name) + '</div><b>Hola, ' + esc(firstName(p.name)) + '</b></div>' +
...
function parentHome(p) {
  ...
  const todayReqs = S.requests.filter((r) => kids.some((k) => k.id === r.studentId) && r.date === today && r.status !== 'cancelada');
  ...
  h += kids.map((k) => {
    ...
    return '<div class="card kid"><div class="avatar">' + k.emoji + '</div><div class="grow"><b>' + esc(k.name) + '</b><div class="muted small">' + esc(k.grade) + ' · ' + esc(levelName(k.level)) + '</div>' +
  ...
  if (auths.length) {
    h += '<div class="section-title">Autorizado(a) para retirar otros niños</div>' + auths.map((a) => {
      const s = a.student;
      return '<div class="card"><div class="row"><span class="avatar sm">' + s.emoji + '</span><div><b>' + esc(s.name) + '</b> <span class="muted small">' + esc(s.grade) + '</span><div class="small">' + kindBadge(a.auth.type) + (a.auth.type === 'temporal' ? ' <span class="muted">' + a.auth.from + ' → ' + a.auth.to + '</span>' : '') +
        ' <span class="muted">· por ' + esc(person(a.auth.createdBy).name) + '</span></div></div></div></div>';
    }).join('');
  }
...
function parentRequests(p) {
  ...
  const list = S.requests.filter((r) => kids.includes(r.studentId));
...
function parentAuths(p) {
  ...
  h += kids.map((k) => {
    const tit = k.titulares.map((t) => '<div class="row item"><span class="avatar sm">👤</span><div><b>' + esc(person(t).name) + '</b> <span class="muted small">' + esc(person(t).relation) + ' · ' + esc(person(t).phone) + '</span></div>' + kindBadge('titular') + '</div>').join('');
    const auths = authsForStudent(k.id).map((a) => {
      const pr = person(a.personId);
      const active = isAuthActive(a);
      let vig = '';
      if (a.type === 'temporal') vig = a.from + ' → ' + a.to;
      if (a.type === 'una_vez') vig = a.used ? 'ya usada' : 'pendiente de uso';
      return '<div class="row item' + (active ? '' : ' inactive') + '"><span class="avatar sm">🧑</span><div><b>' + esc(pr.name) + '</b> <span class="muted small">' + esc(pr.relation) + ' · céd. ' + esc(pr.cedula) + (pr.account ? ' · 📱 tiene cuenta' : '') + '</span><div class="small">' + kindBadge(a.type) + ' <span class="muted">' + vig + (active ? '' : ' · inactiva') + '</span></div></div>' +
        '<button class="btn tiny danger" data-action="revokeAuth" data-id="' + a.id + '" title="Revocar">✕</button></div>';
    }).join('');
    return '<div class="card"><div class="row"><span class="avatar sm">' + k.emoji + '</span><b>' + esc(k.name) + '</b> <span class="muted small">máx. ' + S.settings.maxTitulares + ' titulares</span></div>' + tit + (auths || '<div class="muted small">Sin personas autorizadas.</div>') + '</div>';
  }).join('');
  return h;
}
function parentNotifs(p) {
  const list = S.notifications.filter((n) => n.to === p.id).slice().reverse();
...
function requestCard(r, o = {}) {
  ...
  if (r.kind === 'salida') {
    main = '<b>🚪 Salida ' + esc(fmtDate(r.date)) + ' · ' + fmtTime(r.time) + '</b><div class="small">Retira: ' + esc(describePickup(r)) + ' ' + (r.pickupKind ? kindBadge(r.pickupKind) : '') + '</div>' +
      ...
      (r.status === 'retirado' ? '<div class="small">Salió a las ' + fmtClock(r.exitAt) + (r.exitBy ? ' · confirmó ' + esc(staffMember(r.exitBy).name) : '') + '</div>' : '') +
      (r.status === 'rechazada' ? '<div class="small danger-text">Motivo: ' + esc(r.rejectReason) + '</div>' : '');
  } else {
    main = '<b>📝 Excusa · ' + esc(r.excusaType) + ' · ' + esc(fmtDate(r.date)) + '</b><div class="small">' + esc(r.reason) + '</div>' + (r.attachment ? '<div class="small">📎 ' + esc(r.attachment) + '</div>' : '') +
      (r.status === 'rechazada' ? '<div class="small danger-text">Motivo: ' + esc(r.rejectReason) + '</div>' : '');
  }
...
function viewWhatsapp() {
  const phones = S.persons.filter((p) => p.phone);
  const key = UI.phoneId;
  ...
  return '<div class="phone-wrap"><div class="sim-bar">📞 Simular teléfono de: <select data-change="setPhone">' +
    phones.map((x) => opt(x.id, x.name + ' · ' + x.phone + (x.account ? '' : ' (sin cuenta)'), x.id === key)).join('') + opt('unknown', 'Número desconocido · +507 6000-0000', key === 'unknown') + '</select></div>' +
    '<div class="phone wa"><div class="wa-header"><span class="wa-back">‹</span><span class="wa-avatar">🏫</span><div><b>' + esc(S.school.short) + ' Salidas</b> <span class="verified">✔</span><div class="small">Cuenta de empresa · en línea</div></div></div>' +
...
function chatChips(p, key) {
  if (!p || !p.account) return ['Hola', 'Necesito retirar a mi hijo'];
  ...
  if (auths.length) {
    const a = auths[0];
    chips.push('A ' + firstName(k.name) + ' lo va a retirar ' + (person(a.personId).relation.toLowerCase() === 'abuela' ? 'la abuela' : person(a.personId).name) + ' a las ' + t2);
...
function sendChat() {
  ...
  if (!text) return;
  input.value = '';
  handleIncoming(UI.phoneId, text);
  save();
  render();
}
...
function availableTabs(staff) { return SCHOOL_TABS.filter(([, , cap]) => !cap || staffCan(staff, cap)); }
function viewSchool() {
  const staff = staffMember(UI.staffId) || S.staff[0];
  ...
  const unread = unreadForStaff(staff);
  const body = {
  ...
  const scope = staff.routeId ? '<span class="muted">solo ' + esc(route(staff.routeId).name) + '</span>' : staff.grades ? '<span class="muted">grados: ' + esc(staff.grades.join(', ')) + '</span>' : '<span class="muted">todos los niveles</span>';
  return '<div class="dash"><aside class="side"><div class="side-brand">🏫 ' + esc(S.school.short) + ' Salidas<div class="small muted">' + esc(S.school.name) + '</div></div>' +
    '<div class="side-user"><label class="small muted">Usuario</label><select data-change="setStaff">' + S.staff.map((s) => opt(s.id, s.name + ' · ' + roleName(s.role), s.id === staff.id)).join('') + '</select>' +
...
function schoolHome(staff) {
  const reqs = visibleRequests(staff);
  const t = todayISO();
  ...
  const notifs = S.notifications.filter((n) => n.toRole === staff.role || n.toStaff === staff.id).slice(-8).reverse();
...
function schoolReqCard(r, staff) {
  const st = student(r.studentId);
  const by = person(r.requestedBy);
  const canApprove = staffCan(staff, 'aprobar');
  const canDecide = staffCan(staff, 'decidir_excusas');
  let body, actions = '';
  if (r.kind === 'salida') {
    ...
    body = '<b>🚪 Salida ' + esc(fmtDate(r.date)) + ' · ' + fmtTime(r.time) + '</b><div class="small">Retira: <b>' + esc(pk.name) + '</b> (' + esc(pk.relation) + ') · céd. ' + esc(pk.cedula) + ' ' + kindBadge(r.pickupKind) + '</div><div class="small muted">Motivo: ' + esc(r.reason) + '</div>' +
      (r.pickupPoint ? '<div class="small">📍 ' + esc(r.pickupPoint) + ' · código <b class="mono">' + r.code + '</b>' + (r.autoApproved ? ' · auto-aprobada' : r.decidedBy ? ' · por ' + esc((staffMember(r.decidedBy) || {}).name || '') : '') + '</div>' : '') +
      (r.confirmation ? '<div class="small">Confirmación titular: <b>' + r.confirmation.status + '</b></div>' : '') +
      (r.status === 'retirado' ? '<div class="small">Salió ' + fmtClock(r.exitAt) + ' · garita: ' + esc(staffMember(r.exitBy).name) + '</div>' : '') +
      (r.status === 'rechazada' ? '<div class="small danger-text">Rechazo: ' + esc(r.rejectReason) + '</div>' : '');
    if (r.status === 'pendiente' && canApprove) {
      actions = '<div class="actions"><select class="inline" id="pp-' + r.id + '">' + S.school.pickupPoints.map((x) => opt(x, x, x === S.settings.defaultPickupPoint)).join('') + '</select>' +
    ...
    body = '<b>📝 Excusa · ' + esc(r.excusaType) + ' · ' + esc(fmtDate(r.date)) + '</b><div class="small">' + esc(r.reason) + '</div>' + (r.attachment ? '<div class="small">📎 <a href="#" onclick="return false">' + esc(r.attachment) + '</a></div>' : '<div class="small muted">Sin adjunto</div>') +
      (r.status === 'rechazada' ? '<div class="small danger-text">Rechazo: ' + esc(r.rejectReason) + '</div>' : '');
  ...
  return '<div class="card req"><div class="row top"><span class="avatar sm">' + st.emoji + '</span><div class="grow"><div class="small muted">' + esc(st.name) + ' · ' + esc(st.grade) + ' ' + esc(levelName(st.level)) + ' · vía ' + CHANNEL[r.channel] + ' · solicitó ' + esc(by.name) + ' (' + esc(by.relation) + ') · ' + fmtTs(r.createdAt) + '</div>' + body + '</div>' + badge(r.status) + '</div>' +
...
function schoolRequests(staff) {
  const f = UI.filter;
  let list = visibleRequests(staff).filter((r) => r.kind === 'salida');
...
function schoolGate(staff) {
  const t = todayISO();
  const list = S.requests.filter((r) => r.kind === 'salida' && r.date === t && ['aprobada', 'retirado'].includes(r.status)).sort((a, b) => a.time.localeCompare(b.time));
  ...
  const card = (r) => {
    ...
    return '<div class="card gate ' + r.status + '"><div><div class="gate-time">' + fmtTime(r.time) + '</div>' + personDoc(pk) + '</div><div class="grow"><div class="row"><span class="avatar sm">' + st.emoji + '</span><b>' + esc(st.name) + '</b> <span class="muted small">' + esc(st.grade) + ' · ' + esc(levelName(st.level)) + '</span></div>' +
...
function schoolExcusas(staff) {
  const list = visibleRequests(staff).filter((r) => r.kind === 'excusa');
...
function schoolStudents(staff) {
  ...
  return '<h2>Estudiantes y familias</h2>' + S.levels.map((lv) => {
    const kids = scope.filter((s) => s.level === lv.id);
    ...
    return '<h3>' + esc(lv.name) + '</h3><table class="tbl"><tr><th>Estudiante</th><th>Grado</th><th>Titulares</th><th>Autorizados vigentes</th><th>Bus</th><th>Solicitudes</th></tr>' + kids.map((k) =>
      ...
      '<td>' + (authsForStudent(k.id).filter(isAuthActive).map((a) => esc(person(a.personId).name) + ' ' + kindBadge(a.type)).join('<br>') || '<span class="muted">—</span>') + '</td><td>' + busChip(k) + '</td><td>' + S.requests.filter((r) => r.studentId === k.id).length + '</td></tr>').join('') + '</table>';
  }).join('');
}
function schoolAuths(staff) {
  const scope = new Set(staffScope(staff).map((s) => s.id));
  const list = S.authorizations.filter((a) => scope.has(a.studentId));
  return '<h2>Personas autorizadas</h2><table class="tbl"><tr><th>Persona</th><th>Cédula</th><th>Estudiante</th><th>Tipo</th><th>Vigencia</th><th>Registró</th><th>Estado</th><th></th></tr>' + list.map((a) => {
    ...
    const vig = a.type === 'temporal' ? a.from + ' → ' + a.to : a.type === 'una_vez' ? (a.used ? 'usada ' + fmtTs(a.usedAt) : 'pendiente de uso') : 'permanente';
    const estado = a.revoked ? '<span class="badge st-rechazada">Revocada</span>' : isAuthActive(a) ? '<span class="badge st-aprobada">Activa</span>' : '<span class="badge st-cancelada">Inactiva</span>';
    return '<tr><td>' + esc(p.name) + ' <span class="muted small">' + esc(p.relation) + (p.account ? ' · 📱 cuenta' : '') + '</span></td><td class="mono">' + esc(p.cedula) + '</td><td>' + esc(s.name) + ' <span class="muted small">' + esc(s.grade) + '</span></td><td>' + kindBadge(a.type) + '</td><td>' + vig + '</td><td>' + esc(person(a.createdBy).name) + '</td><td>' + estado + '</td>' +
      '<td>' + (!a.revoked ? '<button class="btn tiny danger" data-action="revokeAuthSchool" data-id="' + a.id + '">Revocar</button>' : '') + '</td></tr>';
  }).join('') + '</table>';
}
function schoolStaff(staff) {
  ...
  return '<h2>Personal</h2><table class="tbl"><tr><th>Nombre</th><th>Rol</th><th>Cargo</th><th>Alcance</th></tr>' + S.staff.map((s) => '<tr><td>' + esc(s.name) + '</td><td><span class="badge role-' + s.role + '">' + esc(roleName(s.role)) + '</span></td><td>' + esc(s.title || '') + '</td><td>' + (s.routeId ? 'Solo ' + esc(route(s.routeId).name) : s.grades ? esc(s.grades.join(', ')) : 'Todos los niveles') + '</td></tr>').join('') + '</table>' +
    ...
    caps.map(([c, l]) => '<tr><td>' + l + '</td>' + roles.map((r) => '<td class="center"><input type="checkbox" data-change="togglePerm" data-role="' + r + '" data-cap="' + c + '"' + (S.permissions[r][c] ? ' checked' : '') + (r === 'admin' ? ' disabled' : '') + '></td>').join('') + '</tr>').join('') + '</table>' +
...
function schoolConfig() {
  ...
  return '<h2>Configuración</h2><form data-form="saveConfig" class="form"><div class="card">' +
    ...
    '<label>Punto de retiro por defecto <select name="defaultPickupPoint">' + S.school.pickupPoints.map((x) => opt(x, x, x === c.defaultPickupPoint)).join('') + '</select></label>' +
    '</div><div class="card"><h3>Escuela</h3><label>Nombre <input name="schoolName" value="' + esc(S.school.name) + '"></label><label>Teléfono de recepción <input name="schoolPhone" value="' + esc(S.school.phone) + '"></label>' +
    '<label>Puntos de retiro (uno por línea)<textarea name="pickupPoints" rows="3">' + esc(S.school.pickupPoints.join('\n')) + '</textarea></label>' +
    ...
    '<div class="card"><h3>Niveles y grados</h3>' + S.levels.map((l) => '<div class="small"><b>' + esc(l.name) + '</b>: ' + esc(l.grades.join(', ')) + '</div>').join('') + '</div>' +
...
function logTable() {
  return '<table class="tbl"><tr><th>Fecha</th><th>Actor</th><th>Evento</th></tr>' + S.log.slice(0, 200).map((l) => '<tr><td class="mono small">' + fmtTs(l.ts) + '</td><td>' + esc(l.actor) + '</td><td>' + esc(l.text) + '</td></tr>').join('') + '</table>';
}
function viewLog() {
  const notifs = S.notifications.slice().reverse().slice(0, 100);
  return '<div class="page"><div class="cols"><div><h2>📜 Bitácora del sistema</h2>' + logTable() + '</div><div><h2>📲 Notificaciones enviadas</h2>' +
    (notifs.length ? notifs.map((n) => '<div class="card notif"><div class="small muted">' + fmtTs(n.ts) + ' · ' + (n.to ? '→ ' + esc(person(n.to).name) + ' (WhatsApp/App)' : n.toRole ? '→ ' + esc(roleName(n.toRole)) : '→ ' + esc(staffMember(n.toStaff).name)) + '</div>' + esc(n.text) + '</div>').join('') : '<div class="empty">Aún no se ha enviado ninguna notificación.</div>') + '</div></div></div>';
}
...
function personDoc(p) {
  if (p.docImage) return '<img class="doc-thumb" src="' + p.docImage + '" data-action="openDoc" data-id="' + p.id + '" title="Ver documento">';
  return '<div class="doc-placeholder" data-action="openDoc" data-id="' + p.id + '" title="Ver documento">🪪<br>' + esc(p.doc || 'sin foto') + '</div>';
}
function qrBox(r) {
...
function schoolRutas(staff) {
  const can = staffCan(staff, 'marcar_bus');
  const list = visibleRoutes(staff);
  if (!list.length) return '<h2>Rutas de bus</h2><div class="empty">No hay rutas registradas.</div>';
  return '<h2>Rutas de bus <span class="muted small">' + (S.settings.simulateBus ? 'modo demo: GPS simulado en ruta' : 'GPS según horario') + '</span></h2>' + list.map((r) => {
    ...
    const kids = S.students.filter((s) => s.routeId === r.id && (!s.busLegs || s.busLegs.includes(leg)));
    const mon = staffMember(r.monitorId);
    ...
    const head = '<div class="route-head"><div><b style="font-size:16px">🚌 ' + esc(r.name) + '</b> <span class="muted small">placa ' + esc(r.plate) + ' · conductor ' + esc(r.driver) + ' · monitora ' + esc(mon ? mon.name : '—') + '</span>' +
...
function modalSalida(d) {
  ...
  return '<h3>🚪 Solicitar salida temprana</h3><form data-form="newSalida" class="form">' +
    ...
    '<p class="small muted">Si cumples la regla de auto-aprobación (titular, anticipación ≥ ' + S.settings.minAnticipationMin + ' min, persona autorizada vigente) se aprueba al instante.</p>' +
...
function modalAuth(d) {
  ...
  const accounts = S.persons.filter((x) => x.account && x.id !== p.id);
  return '<h3>👥 Autorizar persona para retirar</h3><form data-form="newAuth" class="form">' +
    ...
    (mode === 'nueva'
      ...
      : '<label>Cuenta <select name="personId">' + accounts.map((a) => opt(a.id, a.name + ' · ' + a.phone + ' (' + a.relation + ' de ' + studentsOf(a.id).map((s) => firstName(s.name)).join(', ') + ')', a.id === d.personId)).join('') + '</select></label>') +
...
function modalWhere(d) {
  const st = student(d.id); const w = whereIs(d.id);
  return '<h3>📍 ¿Dónde está ' + esc(firstName(st.name)) + '?</h3><div class="where-box">' + esc(w.text) + '</div>' + (w.location ? '<div style="margin-top:10px">' + locationCard(w.location) + '</div>' : '') +
...
function modalScan(d) {
  if (d.found) {
    ...
    return '<h3>✅ Código válido · ' + r.code + '</h3><div class="row" style="align-items:flex-start; gap:14px">' +
      (pk.docImage ? '<img class="doc-big" style="max-width:220px; margin:0" src="' + pk.docImage + '">' : '<div class="doc-placeholder" style="width:120px;height:120px;font-size:13px">🪪<br>' + esc(pk.doc || 'sin foto') + '</div>') +
      ...
      (el.kind === 'una_vez' && !(r.confirmation && r.confirmation.status === 'confirmada') ? '<div class="small danger-text" style="margin-top:6px">Autorización de una sola vez: falta la confirmación del titular.</div>' : '') + '</div></div>' +
  ...
  const today = S.requests.filter((r) => r.kind === 'salida' && r.status === 'aprobada' && r.date === todayISO());
...
function modalDoc(d) {
  ...
  return '<h3>🪪 ' + esc(p.name) + ' <span class="muted small">' + esc(p.relation) + ' · céd. ' + esc(p.cedula) + '</span></h3>' +
    (p.docImage ? '<img class="doc-big" src="' + p.docImage + '">' : '<div class="empty">Documento registrado: <b>' + esc(p.doc || '—') + '</b><br><span class="small">(dato de ejemplo sin imagen; los autorizados nuevos guardan la foto real)</span></div>') +
    '<div class="actions"><button class="btn primary" data-action="closeModal">Cerrar</button></div>';
}
function modalGuide() {
  return '<h3>📖 Guion sugerido para el demo</h3><ol class="guide">' +
    ...
    '<li><b>Permisos:</b> entra como Prof. Diana Ríos (solo ve 3°) y luego como Administración para editar la matriz de permisos y la regla de auto-aprobación.</li>' +
    ...
    '<li><b>Monitora:</b> entra como Kenia Pérez (solo ve el Bus 12), marca Abordó / Bajó / No abordó e inicia o finaliza el viaje. Escribe "Sofía hoy no va en el bus" desde WhatsApp y mira cómo le llega a la monitora.</li>' +
    ...
    '<li>Activa <b>Vista dividida</b> para ver WhatsApp y el dashboard a la vez.</li></ol>' +
...
const ACTIONS = {
  ...
  resetDemo() { if (confirm('¿Reiniciar el demo con los datos de ejemplo?')) resetDemo(); },
  ...
  chatSend() { sendChat(); },
  chatQuick(el) { handleIncoming(UI.phoneId, el.dataset.text); save(); },
  cancelReq(el) { if (confirm('¿Cancelar esta solicitud?')) cancelRequest(el.dataset.id, UI.parentId); },
  revokeAuth(el) { if (confirm('¿Revocar esta autorización?')) revokeAuthorization(el.dataset.id, UI.parentId); },
  revokeAuthSchool(el) { if (confirm('¿Revocar esta autorización?')) revokeAuthorization(el.dataset.id, null); },
  approve(el) {
    const sel = document.getElementById('pp-' + el.dataset.id);
    approveRequest(el.dataset.id, { by: UI.staffId, pickupPoint: sel ? sel.value : S.settings.defaultPickupPoint });
    toast('Salida aprobada y padres notificados', 'ok');
  },
  acceptExcusa(el) { acceptExcusa(el.dataset.id, UI.staffId); toast('Excusa aceptada', 'ok'); },
  askConfirm(el) { requestConfirmation(el.dataset.id, UI.staffId); toast('Se pidió confirmación a los titulares por WhatsApp', 'ok'); },
  markExit(el) {
    ...
    const res = markExit(r.id, UI.staffId);
    if (!res.ok) { alert(res.error); return; }
    UI.modal = null;
    toast('Salida registrada. Padres notificados.', 'ok');
  },
  whereKid(el) { UI.modal = { type: 'where', data: { id: el.dataset.id } }; logEvent('Consultó la ubicación de ' + student(el.dataset.id).name + ' desde la app', person(UI.parentId).name); },
  noBusKid(el) { if (confirm('¿Avisar a la monitora que hoy no va en el bus (ida y vuelta)?')) { markNoBus(el.dataset.id, UI.parentId, ['ida', 'vuelta']); toast('Aviso enviado a la monitora', 'ok'); } },
  openDoc(el) { UI.modal = { type: 'doc', data: { id: el.dataset.id } }; },
  board(el) { markBoarding(el.dataset.route, el.dataset.leg, el.dataset.id, el.dataset.status, UI.staffId, el.dataset.stop || null); },
  tripStatus(el) { setTripStatus(el.dataset.route, el.dataset.leg, el.dataset.status, UI.staffId); },
};
function onClick(e) {
  const el = e.target.closest('[data-action]');
  if (!el) {
    if (e.target.id === 'modal') { UI.modal = null; renderModal(); }
    return;
  }
...
const FORMS = {
  newSalida(d) {
    createRequest({ kind: 'salida', channel: 'web', requestedBy: UI.parentId, studentId: d.studentId, date: d.date, time: d.time, pickupBy: d.pickupBy, reason: d.reason });
    UI.modal = null;
    UI.parentTab = 'solicitudes';
    toast(S.requests[0].status === 'aprobada' ? 'Solicitud auto-aprobada ✅' : 'Solicitud enviada, pendiente de la escuela', 'ok');
  },
  newExcusa(d, form) {
    const file = form.querySelector('input[name=attachment]');
    const name = file && file.files && file.files[0] ? file.files[0].name : null;
    createRequest({ kind: 'excusa', channel: 'web', requestedBy: UI.parentId, studentId: d.studentId, date: d.date, excusaType: d.excusaType, reason: d.reason, attachment: name });
    UI.modal = null;
    UI.parentTab = 'solicitudes';
    toast('Excusa enviada', 'ok');
  },
  newAuth(d, form) {
    ...
    const fileEl = form.querySelector('input[name=docFile]');
    const file = fileEl && fileEl.files && fileEl.files[0];
    if (d.mode === 'nueva' && !file) { alert('Sube una foto de la persona o de su cédula.'); return; }
    const go = (docImage) => {
      const docInfo = file ? { doc: file.name, docImage } : {};
      addAuthorization({
        studentIds: ids, createdBy: UI.parentId, type: d.type, from: d.from, to: d.to,
        personId: d.mode === 'cuenta' ? d.personId : null,
        newPerson: d.mode === 'nueva' ? Object.assign({ name: d.name, relation: d.relation, cedula: d.cedula, phone: d.phone }, docInfo) : (file ? docInfo : null),
      });
      UI.modal = null;
      UI.parentTab = 'autorizados';
      toast('Autorización guardada', 'ok');
      save();
      render();
    };
    if (file) { const fr = new FileReader(); fr.onload = () => shrinkImage(fr.result, go); fr.readAsDataURL(file); } else go(null);
  },
  scan(d) {
    const code = String(d.code || '').replace(/\D/g, '');
    const req = S.requests.find((r) => r.kind === 'salida' && r.status === 'aprobada' && r.date === todayISO() && r.code === code);
    if (!req) { alert('Código no válido o sin salida aprobada para hoy.'); return; }
    logEvent('Escaneó el código ' + code + ' (' + student(req.studentId).name + ')', staffMember(UI.staffId).name);
    UI.modal = { type: 'scan', data: { found: req.id } };
  },
  reject(d) { rejectRequest(d.id, d.reason, UI.staffId); UI.modal = null; toast('Rechazada y padres notificados', 'ok'); },
  saveConfig(d) {
    S.settings.autoApprove = d.autoApprove === 'on';
    S.settings.minAnticipationMin = Math.max(0, +d.minAnticipationMin || 0);
    S.settings.maxTitulares = Math.max(1, +d.maxTitulares || 2);
    S.settings.schoolStart = d.schoolStart || S.settings.schoolStart;
    S.settings.schoolEnd = d.schoolEnd || S.settings.schoolEnd;
    S.settings.simulateBus = d.simulateBus === 'on';
    S.settings.busProgress = Math.max(0, Math.min(1, (+d.busProgress || 0) / 100));
    S.settings.newAuthDays = Math.max(0, +d.newAuthDays || 0);
    S.school.name = d.schoolName;
    S.school.phone = d.schoolPhone;
    S.school.pickupPoints = d.pickupPoints.split('\n').map((x) => x.trim()).filter(Boolean);
    S.settings.defaultPickupPoint = S.school.pickupPoints.includes(d.defaultPickupPoint) ? d.defaultPickupPoint : S.school.pickupPoints[0];
    logEvent('Actualizó la configuración (auto-aprobación: ' + (S.settings.autoApprove ? 'activa' : 'inactiva') + ', anticipación ' + S.settings.minAnticipationMin + ' min)', staffMember(UI.staffId).name);
...
function onChange(e) {
  ...
  if (k === 'setParent') { UI.parentId = el.value; UI.parentTab = 'inicio'; }
  else if (k === 'setPhone') { UI.phoneId = el.value; }
  else if (k === 'setStaff') { UI.staffId = el.value; const s = staffMember(el.value); UI.schoolTab = s.role === 'garita' ? 'salidas_hoy' : s.role === 'monitora' ? 'rutas' : 'inicio'; }
  else if (k === 'busProgress') { S.settings.busProgress = Math.max(0, Math.min(1, (+el.value || 0) / 100)); }
  else if (k === 'togglePerm') { S.permissions[el.dataset.role][el.dataset.cap] = el.checked; logEvent('Permiso "' + el.dataset.cap + '" ' + (el.checked ? 'otorgado a' : 'quitado a') + ' ' + roleName(el.dataset.role), staffMember(UI.staffId).name); save(); return; }
  else if (k === 'modalField') {
    const form = el.closest('form');
    UI.modal.data = Object.assign({}, UI.modal.data, formData(form));
    renderModal();
    return;
  }
  save();
