...
**IP, geolocalización y huella del dispositivo (Task 15).** `/super` también muestra desde dónde y con qué se conecta cada probador: IP, "Ciudad, País" (solo en Vercel, leído de `x-vercel-ip-country/-city/-latitude/-longitude/-timezone`; en local siempre `null`, no hay forma honesta de saberlo fuera del edge) y un bloque **Dispositivos** con cada huella distinta que ese probador ha usado (pantalla, plataforma, navegador), con primera y última vez. La huella la calcula el navegador (`public/client/telemetry.js`, y `public/client/super.js` para la sesión del propio super admin) con señales normales — tamaño de pantalla, zona horaria, idioma, `navigator.userAgentData`… — resumidas en un hash corto (`fp`, SHA-256 vía `crypto.subtle`); nunca usa canvas/audio fingerprinting ni nada de terceros. El servidor solo guarda una lista blanca de esas claves (el resto se descarta) y solo en el evento `session_start`; `fp` tiene su propia columna (`activity_events.fp`, migración `010_activity_fingerprint`) para poder agrupar dispositivos con un índice. El aviso push de entrada añade " · Ciudad, País" al final cuando se conoce. **Privacidad:** son datos técnicos de los probadores del piloto (personas reales de la escuela) para depurar el piloto, viven solo en `activity_events`, se borran con el purge de `/super` (**🧹 Borrar actividad de más de 30 días**) y no se muestran a nadie fuera del super admin.
