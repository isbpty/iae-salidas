...
### Task 14: `/super` instalable en el celular con notificaciones push cuando entra un probador

**Pedido por el usuario el 2026-09-22 (sustituye la idea del correo):** Isaac quiere una app en su celular, solo la página `/super`, que le notifique cuando un probador inicia sesión, para abrir el panel y observar.
**Excepción a "sin dependencias nuevas":** se añade `web-push` (implementación estándar de VAPID + cifrado del Web Push Protocol); nada más.
**Files:** Create `public/super-manifest.webmanifest` (nombre "IAE Super", `start_url: /super`, `display: standalone`, iconos SVG/PNG en `public/icons/`), `public/super-sw.js` (service worker: `push` → `showNotification` con título, cuerpo y `data.url`; `notificationclick` → abre o enfoca `/super`), `server/transports/push.js` (`WebPushSender` con `web-push`, `MemoryPush` para tests; `send(subscription, payload)`; ante 404/410 borra la suscripción), migración `009_push_subscriptions` (`push_subscriptions(id, tester_id, endpoint UNIQUE, keys jsonb, ua, created_at, last_ok_at, failures int)`), `server/config.js` (`vapidPublicKey`, `vapidPrivateKey`, `vapidSubject` = `mailto:` del super admin; si faltan, la función está apagada), rutas en `server/app.js` bajo la cookie de super: `GET /api/super/push/config` → `{ enabled, publicKey, subscribed }`, `POST /api/super/push/subscribe { subscription }`, `POST /api/super/push/unsubscribe`, `POST /api/super/push/test` (envía una notificación de prueba); disparo: tras un `auth/login` exitoso con `testerId` no nulo (no en `auth/switch`), enviar a **todas** las suscripciones "Probador X entró como Y (rol) · HH:MM" con `url: /super?tester=<id>`; cooldown de 10 min por probador (`app_meta` `push_alert_<testerId>`); sin bloquear la respuesta más de 3 s, fallos a `console.error` y evento de actividad `kind: 'push'`; `server/index.js` y `api/index.js` inyectan `push` en deps; `public/super.html` (`<link rel="manifest">`, `theme-color`, meta `apple-mobile-web-app-capable`, registro del service worker), `public/client/super.js` (bloque "Avisos en el celular": estado, botón "Activar avisos en este dispositivo" → `Notification.requestPermission` + `pushManager.subscribe` con la clave pública, botón "Probar", "Desactivar", y ayuda para iPhone: "añade a pantalla de inicio primero"); `vercel.json` (servir `/super-sw.js` con `Service-Worker-Allowed: /` y `Cache-Control: no-cache`; el manifest con su tipo); `README.md`/`VERCEL-DEPLOY.md` (generar claves con `npx web-push generate-vapid-keys`, variables `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`).
**Tests** `server/push.test.js` con `MemoryPush`: login de probador envía 1 push a cada suscripción con probador, usuario, rol y hora Panamá; segundo login del mismo probador dentro de 10 min no envía; `switch` no envía; sin claves VAPID no envía y no falla; un 410 del proveedor borra la suscripción; las rutas exigen la cookie de super; el login no se ve afectado si el envío falla.

### Task 15: IP, geolocalización y huella del dispositivo en cada sesión

**Pedido por el usuario el 2026-09-23:** "guardar IP, location de la IP, etc., cualquier fingerprint" para saber desde dónde y con qué dispositivo entra cada probador, visible en `/super`.
**Files:** Modify `server/activity.js` (`clientInfo(req)` → `{ ip, ua, geo: { country, region, city, lat, lng, tz } }` leyendo las cabeceras de Vercel `x-vercel-ip-country`, `x-vercel-ip-country-region`, `x-vercel-ip-city` (URL-decoded), `x-vercel-ip-latitude`, `x-vercel-ip-longitude`, `x-vercel-ip-timezone`, y `x-forwarded-for` solo en serverless; en local `geo` nulo), `server/app.js` (los eventos `login`, `login_failed`, `pin`, `switch_user`, `super_login` guardan `geo` en `data` y el `ip`/`ua` ya existentes; la notificación push incluye ciudad/país si existe), `server/activity-queries.js` (`sessions` y `testers` del `summary` exponen `lastIp`, `lastGeo`, `lastDevice`; nuevo bloque `devices` por probador: lista de huellas distintas con primera y última vez), `public/client/telemetry.js` (`session_start` envía la huella: `screen.width/height`, `devicePixelRatio`, `colorDepth`, `navigator.language/languages`, `platform`, `hardwareConcurrency`, `deviceMemory`, `maxTouchPoints`, `Intl.DateTimeFormat().resolvedOptions().timeZone`, `navigator.connection.effectiveType`, `navigator.userAgentData` (brands, mobile, platform) si existe, `window.innerWidth/innerHeight`, `standalone` (PWA), y un hash corto `fp` de todo eso con SHA-256 vía `crypto.subtle` para agrupar; también en `super.js` para la sesión de `/super`), `server/activity.js` (`ingestClientEvents`: `session_start` se guarda con `data` completo sin tope de 2 KB pero con lista blanca de claves; el `fp` se copia a la columna nueva `activity_events.fp text` en migración `010_activity_fingerprint` con índice `(tester_id, fp)`), `public/client/super.js` (en cada tarjeta de probador: "Último acceso: Ciudad, País · IP · dispositivo (plataforma, navegador, pantalla)"; en la línea de tiempo los `login` muestran ciudad y dispositivo; bloque "Dispositivos" por probador con las huellas distintas y sus fechas; en el CSV, columnas `ip`, `city`, `country`, `fp`).
**Privacidad:** son datos de probadores del piloto (personas de la escuela), se guardan solo en `activity_events`, se borran con el purge de `/super`, y no se muestran a nadie fuera del super admin. Documentar en README.
**Tests** `server/activity.test.js` (cabeceras de Vercel → `geo` en el evento de login; sin cabeceras → nulo; `session_start` con huella → `fp` en columna; claves no permitidas descartadas), `server/activity-queries.test.js` (`summary.testers[].lastGeo/lastDevice` y `devices`), `server/push.test.js` (el cuerpo del push incluye "· Ciudad, País" cuando hay geo).
