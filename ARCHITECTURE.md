...
- **Cliente**: `public/index.html` + `public/client/*.js`. Solo pinta la proyección y envía comandos; sin lógica de negocio.
- **Tiempo real**: SSE en local; en Vercel sondeo cada 3 s con `If-None-Match` (304 si nada cambió).
- **Seguridad**: cookie HMAC, PIN con límite de intentos, secretos obligatorios, estáticos restringidos a `public/` (`index.html` y `client/`), adjuntos servidos solo a quien puede verlos.
