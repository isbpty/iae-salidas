...
import { applyPrototypeCommand } from './prototype-commands.js';
...
async function api(req,res,url){
 ...
 if(url.pathname==='/api/prototype-state'&&req.method==='GET')return json(res,200,{state:store.state.prototypeState,revision:store.state.prototypeRevision||0});
 if(url.pathname==='/api/prototype-state'&&req.method==='PUT'){const input=await body(req);if(!input.state||typeof input.state!=='object')return json(res,400,{error:'invalid_state'});const allowed=['parent','reception','gate','admin','teacher','monitora'];if(!allowed.includes(user.role))return json(res,403,{error:'forbidden'});const saved=await store.mutate(s=>{if(input.expectedRevision!=null&&input.expectedRevision!==(s.prototypeRevision||0))throw Object.assign(new Error('prototype_revision_conflict'),{status:409});validatePrototypeMutation(s.prototypeState,input.state,user);s.prototypeState=input.state;s.prototypeRevision=(s.prototypeRevision||0)+1;s.audit.unshift({id:crypto.randomUUID(),at:new Date().toISOString(),actorId:user.id,actorRole:user.role,action:input.initialize?'prototype.initialized':'prototype.updated',entity:'prototype:shared'});return {ok:true,revision:s.prototypeRevision}});publish({type:'prototype.changed',actorId:user.id});return json(res,200,saved)}
 if(url.pathname==='/api/prototype-command'&&req.method==='POST'){const input=await body(req);const saved=await store.mutate(s=>{if(input.expectedRevision!==(s.prototypeRevision||0))throw Object.assign(new Error('prototype_revision_conflict'),{status:409});s.prototypeState=applyPrototypeCommand(s,user,input.command||{});s.prototypeRevision=(s.prototypeRevision||0)+1;s.audit.unshift({id:crypto.randomUUID(),at:new Date().toISOString(),actorId:user.id,actorRole:user.role,action:`prototype.command.${input.command?.type||'unknown'}`,entity:'prototype:shared'});return {ok:true,revision:s.prototypeRevision,state:s.prototypeState}});publish({type:'prototype.changed',actorId:user.id});return json(res,200,saved)}
